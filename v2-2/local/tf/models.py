...
# Modified by M.W. Mak in Mar 2020 to the make_embedding() so that it can read a subsegment as
# specified in the range parameter in the .scp file
...
class Model(object):
    ...
    def load_model(self, sess, input_dir, logger):
        ...
        # Kaldi in v2-1 only have one embedding layer.
    ...
    # Modified by M.W. Mak to read subsegments as defined in the range parameters in .scp file        
    def make_embedding(self, input_stream, output_stream, model_dir, min_chunk_size, chunk_size, use_gpu, logger):
        ...
        with tf.Session(config=config) as sess:
            ...
            # For each line in the .scp file, read the corresponding .ark file at the offset and range.
            for scp_line in input_stream:
                (key, mat) = kaldi_io.read_mat_ark_range(scp_line)
...
class ModelL2LossWithoutDropoutLReluAttention(Model):
    ...
    def build_model(self, num_classes, input_feature_dim, output_dir, logger=None):
        ...
        # Same size as the DNN in Kaldi (v2-1)
        layer_sizes = [512, 512, 512, 512, 1500]
        kernel_sizes = [5, 5, 7, 1, 1]
        embedding_sizes = [128]
        ...
        tf.reset_default_graph()
        #tf.compat.v1.reset_default_graph()       # Tensorflow 2.0 (M.W. Mak)
        self.graph = tf.Graph()
