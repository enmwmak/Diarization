...
# Now we create the nnet examples using local/tf/get_egs.sh, which will
# call local/tf/create_egs.py to create the range files in egs/temp and
# call local/tf/create_tar_files.py to create the .tar and .npy files in egs/.
...
#
# Changed by M.W. Mak
# Divide training of x-vector extractor into archive creation (stage=4) and
# gradient descent training (stage=6). 
#
if [ ${stage} -eq 6 ]; then
...
#     --tf-model-class="ModelWithoutDropout" \       # Used in the original script (Mak)
# xvector_tf_attent_3ep is creating by setting --num-epochs=3

# Use Attention model (Mak, 21/11/19)
if [ ${stage} -eq 7 ]; then
  local/tf/train_dnn.py \
    --stage=${train_stage} \
    --tf-model-class="ModelL2LossWithoutDropoutLReluAttention" \
