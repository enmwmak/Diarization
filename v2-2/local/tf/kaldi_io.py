...
# Modified by M.W. Mak in Mar 2020 to handle the range information in the .scp file
...
# Added by M.W. Mak to read a subsegment as defined in the range parameter
def read_mat_ark_range(scp_line):
    key, _ = scp_line.split(' ', 1)
    print('key = %s' % key)
    try:
        (fd, range) = open_ark_with_range(scp_line)
        mat = read_mat(fd)
        return key, mat[range[0]:range[1]+1, :]
    except:
        print('Error in kaldi_io.py read_mat_ark_range()')


# Added by M.W. Mak to handle the range info in the .scp file
# Eg. iaaa_000-00000075-00000225 xvector_cmvn_feats_callhome1.1.ark:9[75:224],
# where 9 is the offset and [75:224] is the range to be read
# The function returns both fd and the range in a 2-element list (begin, end)
def open_ark_with_range(scpline, mode='rb'):
    offset = None
    range = [0,0]
    try:
        # strip 'ark:' prefix from r{x,w}filename (optional),
        if re.search('^(ark|scp)(,scp|,b|,t|,n?f|,n?p|,b?o|,n?s|,n?cs)*:', scpline):
            (prefix, scpline) = scpline.split(':', 1)
        # separate offset and range from filename
        print(scpline)
        if re.search(':[0-9]+\[[0-9]+:[0-9]+\]',scpline):
            (key_and_file, offset_range) = scpline.split(':', 1)
            (_, file) = key_and_file.split(' ', 1)
        # separate offset from range
        if re.search('[0-9]+\[[0-9]+:[0-9]+\]',offset_range):
            (offset, range) = offset_range.split('[', 1)
            range = range.rstrip()
            range = range[:-1]
            (begin, end) = range.split(':', 1)
