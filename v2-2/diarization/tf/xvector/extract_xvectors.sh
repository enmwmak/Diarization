...
# Modified by M.W. Mak on Mar 2020 to use BUT x-vector extractor.
...
for f in $srcdir/model_final/model.meta $srcdir/min_chunk_size $srcdir/max_chunk_size $data/feats.scp ; do
...
if $apply_cmn; then
  ...
  feature_rspecifier="ark,s,cs:apply-cmvn-sliding --norm-vars=false --center=true --cmn-window=300 scp:${sub_sdata}/JOB/feats.scp ark:- |" 
else
  #feats="scp:${sub_sdata}/JOB/feats.scp"
  feature_rspecifier="scp:${sub_sdata}/JOB/feats.scp"  
fi

if [ $stage -le 1 ]; then
  echo "$0: extracting xvectors from the BUT x-vector extractor in final_model/"
  if $use_gpu; then
    for g in $(seq $nj); do
