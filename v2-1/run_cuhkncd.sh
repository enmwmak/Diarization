...
# Modified by M.W. Mak in Aug. 2019
# Performance in DER
#   VAD      Part  No PLDA Adapt                     w/ PLDA Adapt
#                  DER    Miss    FA    Spk-err
# --------------------------------------------------------------------
#   Aspire   0     17.01  10.80   1.00  5.20
#   Aspire   1     16.96  10.50   0.90  5.50
...
#========================================================================
# Prepare features (compute MFCCs). No GPU required. Fastest on enmcomp2
# with 16 cores. On other hosts, set -nj <no. of cores in CPU>
#========================================================================
if [ $stage -eq 1 ]; then
...
if [ $stage -eq 7 ]; then
  ...
  # "segments" file in $nnet_dir/xvectors_cuhkncd${p} and 2. This will make this file contains
  # the same number of lines as $nnet_dir/xvectors_cuhkncd${p}/xvectors.scp
...
#============================================================================================
if [ $stage -eq 8 ]; then
  # Train a PLDA model using SRE data but use cuhkncd${p} data for whitening
...
if [ $stage -eq 11 ]; then
  ...
  diarization/cluster.sh --cmd "$train_cmd --mem 4G" \
    ...
    $nnet_dir/xvectors_cuhkncd${p}_segmented/plda_scores $nnet_dir/xvectors_cuhkncd${p}_segmented/plda_scores_num_spk

  # Compare the diarization decisions in $nnet_dir/xvectors_cuhkncd${p}_segmented/plda_scores_num_spk/rttm
  # with data/cuhkncd${p}/cuhkncd_ref.rttm
  ...
  echo "DER: $der%"
  miss=`cat $nnet_dir/results/DER_num_spk.txt | grep 'MISSED\ SPEAKER\ TIME' |  cut -d'(' -f2 | awk '{print $1}'`
  echo "MISS: $miss%"
  fa=`cat $nnet_dir/results/DER_num_spk.txt | grep 'FALARM\ SPEAKER\ TIME' |  cut -d'(' -f2 | awk '{print $1}'`
  echo "FA: $fa%"
  sr=`cat $nnet_dir/results/DER_num_spk.txt | grep 'SPEAKER\ ERROR\ TIME' |  cut -d'(' -f2 | awk '{print $1}'`
  echo "Spk-err: $sr%"  
